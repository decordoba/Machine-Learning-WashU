...
def hashfeatures(name):
  B = 768; # number of dimensions in our feature space
  v = [0] * B; # initialize the vector to be all-zeros
  name = name.lower() # make all letters lower case
  # hash prefixes & suffixes
  for m in range(3):
    # This records the "first syllable" aprox
    featurestring = 'prefix' + name[0:min(m + 1, len(name))]
    v[hash(featurestring) % B] = 1
    # This records the "second syllable" aprox
    featurestring = 'posfix' + name[2:min(m + 3, len(name))]
    v[hash(featurestring) % B] = 1
    # This records the "last syllable" aprox
